...
#ifdef SHARE_DEBUG
	#define sharelog(args...) debuglog(args)
#else
	#define sharelog(args...)
#endif
...
//	check_job(job);
	...
	if(!worker)
	{
		...
		sharelog("new worker for job=%i, valid=%i\n", job ? job->id : 0, valid);
	}
	else
	{
		sharelog("found worker for job=%i, valid=%i\n", job ? job->id : 0, valid);
	}
...
//	check_job(job);
	...
	sharelog("Adding job=%i, valid=%i, share_diff=%f, error_number=%i\n", 
		job ? job->id : 0, valid, share_diff, error_number);
