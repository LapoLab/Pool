...
#if 0
#include <sys/syscall.h> // for lyra2zz debugging only
#endif
...
static inline void revend4(char *be, const char *src)
{
...
static void lyra2zz_build_header_stream(YAAMP_JOB_VALUES *submitvalues, YAAMP_JOB_TEMPLATE *templ, const char *nonce, const char *ntime)
{
//	char nonce_be[12] = { 0 };
	char ntime_be[12] = { 0 };
	char version_be[12] = { 0 };
	char nbits_be[12] = { 0 };

//	revend4(nonce_be, nonce);

	//revend4(ntime_be, ntime);
	revend4(version_be, templ->version);
	revend4(nbits_be, templ->nbits);
	
	sprintf(submitvalues->header, "%s%s%s%s%s%s%s", version_be, templ->prevhash_be, submitvalues->merkleroot_be,
		ntime, nbits_be, nonce, templ->accumulatorcheckpoint_be);	
}
	...
	const char *nonce1, const char *nonce2, const char *ntime, const char *nonce)
{
	...
	bool is_lyra2zz = !strcmp(g_stratum_algo, "lyra2zz"); 

	if (is_lyra2zz) {
		strncpy(submitvalues->merkleroot_be, merkleroot.c_str(), 64);
	} else {
	...
	if (!strcmp(g_stratum_algo, "lbry")) {
		...
		// note that accumulatorcheckpoint endianness differs from
		// prevhash and merkleroot.
		lyra2zz_build_header_stream(submitvalues, templ, nonce, ntime);
		memcpy(submitvalues->header_be, submitvalues->header, sizeof(submitvalues->header));
...
//	printf("%s\n", submitvalues->header_be);
	...
	if (is_lyra2zz) {
		...
		char debugfname[32] = { 0 };
		sprintf(debugfname, "./hashdump_%i.txt", thr_id);

		FILE *f = fopen(debugfname, "ab+");

		if (f) {
			fprintf(f, "LYRA2ZZ HEADER: %s\nHASH(nonce = %s): %s\n\n", 
				submitvalues->header, 
				nonce, 
...
#ifdef CLIENTSUBMIT_DEBUG
#define cslog(args...) debuglog(args); stratumlog(args)
#else
#define cslog(args...) stratumlog(args)
#endif
	...
	char *extranonce2, char *ntime, char *nonce, char *vote)
{
	...
	cslog("Processing auxs: 0x%x. block_size = 0x%x\n", templ->auxs_size, block_size);
	...
	if(hash_int <= coin_target)
	{
		...
		if(b)
		{
		        cslog("*** ACCEPTED %s %d (diff %g) by %s (id: %d)\n", coind->name, templ->height,
...
bool client_submit(YAAMP_CLIENT *client, json_value *json_params)
{
	...
	if (templ->height && (!strcmp(g_current_algo->name,"lyra2z") || !strcmp(g_current_algo->name, "lyra2zz"))) {
